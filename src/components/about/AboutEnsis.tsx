        ...
        <Container className="overflow-hidden rounded-2xl border border-[#e2d8cb] bg-white shadow-sm px-0! py-0!">
          ...
          <div className="grid grid-cols-2 divide-y divide-[#295240] bg-[#0d3828] sm:grid-cols-3 md:grid-cols-5 md:divide-x md:divide-y-0">
            {stats.map((item, index) => (
              <div
                key={index}
                className="flex items-center justify-center px-4 py-2 text-center gap-4"
              >
                <div className="text-2xl text-[#cba25f]">
                  <Image
                    src={item.image}
                    alt={item.label} width={50} height={50} />
                </div>
                <div>
                  <h3 className="text-3xl font-medium text-white text-left">
                    {item.value}
                  </h3>
                  <p className="mt-2 text-xs tracking-wide text-[#d8d2ca] md:text-sm">
                    {item.label}
                  </p>
                </div>
              </div>
