...
const handleRequestOTP = async (e: React.FormEvent) => {
  ...
  const handleVerifyStep = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Try to find token in common response patterns
      ...
      if (token) {
        ...
        setError("Verification successful but reset token was not received.");
      }
    } catch (err: any) {
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post(`${API_URL}/auth/reset-password`, {
        resetToken, // Email aur OTP ki jagah resetToken bhej rahe hain
        newPassword,
      });
